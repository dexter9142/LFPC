...
# Remove large rules (more than 2 states in the right part A->BCD)
    ...
    for let in D[S]:
        ...
        try:
            if not rules[S] and not rules[let]:
                for v in rules[let]:
                    if v not in rules[S]:  # wtf it doesnt work for 1 S
                        rules.setdefault(S, []).append(v)
        except Exception as e:

           return  # I have no f-ing clue how to do this in another way tho
    return rules


def hardReplaceB(rules, let, new_rules):
    new_dict = copy.deepcopy(rules)
    for key in new_dict:
        values = new_dict[key]
        for i in range(len(values)):
            if values ==['B']:
                values = ['AXaD']   #hardcoded, sry, idk
                for j in range(0, len(values[i]) - 2):
                    if j == 0:
                        rules[key][i] = rules[key][i][0] + let[0]
                    else:
